...
const TypeaheadNative = (props) => {
  const { myCustomStandaloneSearchBox, navigation, route, isCreditCustomer = true } = props;
  ...
  const getSuggestions = () => {
    if (searchState?.value) {
      const suggestion = searchState?.groups?.filter((item, index) => searchState?.groups.indexOf(item) === index);
  ...
  //   ) : null;
  // };
  return <Text>UI has rendered. Please use your own search UI here.</Text>;
};
